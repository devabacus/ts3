...
import 'package:ts3_client/ts3_client.dart';
...
class _ServerCheckUiState extends State<ServerCheckUi> {
  ...
  Future<void> _updateItem(TestData item, String newTitle) async {
    setState(() => _isLoading = true);
    try {
      final updatedItem = TestData(id: item.id, title3: newTitle);
  ...
  void _showEditDialog(TestData item) {
    final editController = TextEditingController(text: item.title3);
    showDialog(
      context: context,
